...
> If you need more, you can generate it yourself by using the python script in /scripts.
...
```
Ensure that you are working on the branch "main" to get the latest version or branch "dev" for the latest development version:
```bash
git checkout main
git checkout dev
```
...
> [!NOTE]
> In previous versions, the file was included via ```include_dir_named```, but this sometimes caused confusion with double-included files when having backup files with .yaml extensions (e.g., sungrow.bak.yaml).

> [!WARNING]  
> If you're using an external database like PostgreSQL, make sure to also exclude the following sensors from your recorder configuration. They can contain null bytes (0x00) which can't be stored in PostgreSQL. Not excluding them can prevent **any** changes from being recorded!
> ```yaml
> recorder:
>   exclude:
>     entities:
